...
/*  var vendor_blacklist = $('#vendor_blacklist').dataTable({
  ...
  $('#submit').click(function(){
    $.post('<?php echo base_url();?>vendor_management/vendor_blacklist/save',$('#formData').serialize(),function(){
        //vendor_blacklist.fnClearTable();
      /*  $.getJSON('<?php echo base_url();?>master_data/company/getData',function(data){
          company.fnAddData(data);
          $('.form-input').val('');
        });*/
        window.location.href = "<?php echo site_url('/vendor_management/vendor_blacklist'); ?>";   
    }); 
  });
...
function del(id_vendor_blacklist){
  ...
  if(con==true){
    $.post('<?php echo base_url();?>vendor_management/vendor_blacklist/delete/'+id_vendor_blacklist,function(){
       window.location.href = "<?php echo site_url('/vendor_management/vendor_blacklist'); ?>";  
...
function edit(id_vendor_blacklist){
  $.getJSON('<?php echo base_url();?>vendor_management/vendor_blacklist/getDetail/'+id_vendor_blacklist,function(data){
        ...
        <a data-toggle="modal" href="#myModal" class="btn btn-success">Add New Vendor Blacklist</a><br>
          ...
          <tr>
            ...
            <button class="btn btn-primary btn-xs" onclick="edit('<?php echo $row->id_vendor_blacklist;?>')"><i class="fa fa-edit"></i></button>
             ...
             $status_vendor = $row->status_blacklist;
             if($status_vendor == 'Waiting                       '){?>
            <a href="<?php echo base_url();?>vendor_management/vendor_blacklist/approve/<?php echo $row->id_vendor_blacklist;?>" onclick="return confirm('Are You Sure ?')" class="btn btn-primary btn-xs"><i class="fa fa-check"></i> Approve</a>
            <?php } else if ($status_vendor == 'Approved                      ') { ?>
            <a href="<?php echo base_url();?>vendor_management/vendor_blacklist/cancel/<?php echo $row->id_vendor_blacklist;?>" onclick="return confirm('Are You Sure ?')" class="btn btn-primary btn-xs"><i class="fa fa-check"></i> Cancel</a>
              <a href="<?php echo base_url();?>vendor_management/vendor_blacklist/rejected/<?php echo $row->id_vendor_blacklist;?>" onclick="return confirm('Are You Sure ?')" class="btn btn-primary btn-xs"><i class="fa fa-check"></i> Rejected</a>
            <?php }else{
              echo ' ';
            } ?>
