...
$(document).ready(function(){
  ...
  $('#submit').click(function(){
    $.post('<?php echo base_url();?>master_data/vendor/save',$('#formData').serialize(),function(){
        vendor.fnClearTable();
      /*  $.getJSON('<?php echo base_url();?>master_data/company/getData',function(data){
          company.fnAddData(data);
          $('.form-input').val('');
        });*/
        window.location.href = "<?php echo site_url('/master_data/vendor'); ?>";   
    }); 
  });
...
function del(id_vendor){
  ...
  if(con==true){
    $.post('<?php echo base_url();?>master_data/vendor/delete/'+id_vendor,function(){
      $.getJSON('<?php echo base_url();?>master_data/vendor/getData',function(data){
        var vendor = $('#vendor').dataTable({
            "bPaginate": true,
            "bLengthChange": false,
            "bFilter": false,
...
function edit(id_vendor){
  $.getJSON('<?php echo base_url();?>master_data/vendor/getDetail/'+id_vendor,function(data){
...
<div class="row">
    ...
    <a href="<?php echo $this->page->base_url('/add');?>" class="btn btn-success">Add New vendor</a><br>
          ...
          <tr>
            ...
            <a class="green" href="<?php echo site_url('/vendor/vendor/edit/'.$row->id_vendor); ?>"><button class="btn btn-primary btn-xs"><i class="fa fa-edit"></i></button></a>
              <?php 
             $status_vendor = $row->status_approved;
             if($status_vendor == 'New                           '){?>
            <a href="<?php echo base_url();?>vendor/vendor/approve/<?php echo $row->id_vendor;?>" onclick="return confirm('Are You Sure ?')" class="btn btn-primary btn-xs"><i class="fa fa-check"></i> Approve</a>
