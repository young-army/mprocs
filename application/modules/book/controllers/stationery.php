...
class Stationery extends MX_Controller {
	...
	function submit($trxnum){
		$this->db->query("update stationery_cart set status = '1' where trx_number = '$trxnum'");
		$this->session->set_userdata('trxnum',date('hYimsd'));
		redirect('book/stationery/cart');
	...
	function cart($trxnum = ''){
		...
		if($trxnum==''){
		$data['data']			= $this->db->query("select trx_number as trxnum,status from stationery_cart where userid = '".$sess->id."' group by trxnum,status")->result();
